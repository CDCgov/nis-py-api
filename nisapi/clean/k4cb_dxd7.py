...
def clean(df: pl.LazyFrame) -> pl.LazyFrame:
    return (
        df.pipe(drop_bad_rows, "suppresion_flag")
        .pipe(clean_geography_type, "geography_level")
        .pipe(clean_geography, "geography_name")
        .pipe(clean_domain_type, "demographic_level", append="age")
        .pipe(clean_domain, "demographic_name", donor_colname="age_group")
